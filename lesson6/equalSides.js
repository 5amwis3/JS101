...
Your job is to take that array and 
find an index N where the sum of the integers to the left
...
start iterating at a point
sum up everything to left
sum up everything to right
if they equal eachother, assign to lowestSum
  continue and if it is smaller than lowest sum, asign new value

sub function to add left and right numbers
...
function findEvenIndex(arr) {
  let lowestSum = [];
  let sum = arr => arr.reduce((accu, num) => num + accu, 0);
  arr.forEach((mid, idx) => {
  ...
  if (lowestSum.length === 0) lowestSum.push(-1);
  return Math.min(...lowestSum);
}
